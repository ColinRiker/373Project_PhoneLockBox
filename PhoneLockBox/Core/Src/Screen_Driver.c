...
void screenResolve(void) {
    ...
    case UNLOCKED_EMPTY_AWAKE:
        ...
        if (full_refresh) {
            ...
            // Draw icons
            ILI9341_Draw_Lock(280, 20, 20, YELLOW, false); // Lock icon (unlocked)
            ILI9341_Draw_Phone(10, 10, 20, false); // Phone icon
        }
        break;
    ...
    case UNLOCKED_FULL_AWAKE_FUNC_A:
        ...
        // Display time (always update this part)
        current_time = get_time();
        if (full_refresh || strcmp(prev_time_str, current_time) != 0) {
            // First clear the previous time area
    ...
    case UNLOCKED_FULL_AWAKE_FUNC_B:
        ...
        if (full_refresh) {
            ...
            w = (320 - get_text_width("Press Button to save", FONT4))/2;
            ILI9341_Draw_Text("Press Button to save", FONT4, w, 160, WHITE, BACKG);

            // Draw lock and phone icons
            ILI9341_Draw_Lock(280, 20, 20, YELLOW, false); // Unlocked
            ILI9341_Draw_Phone(10, 10, 20, true); // Phone present

            // Draw circle for dial indicator
            int rad = 40;
            int rot_enc = TIM1->CNT;
            float deg = 360 - (rot_enc/100.0)*360;
            ILI9341_Draw_RingSector(160, 200, rad-3, rad, deg, RED);
        }
        break;
    ...
    case LOCKED_FULL_AWAKE:
        ...
        if (full_refresh) {
            ...
            ILI9341_Draw_Text("Time Remaining", FONT4, w, 80, WHITE, BACKG);
        ...
        // Always update timer display with partial refresh
        current_time = get_time();
        if (full_refresh || strcmp(prev_time_str, current_time) != 0) {
    ...
    case LOCKED_FULL_NOTIFICATION_FUNC_A:
        ...
        if (full_refresh) {
            ...
            // Draw lock and phone icons with indicator
            ILI9341_Draw_Lock(280, 20, 20, YELLOW, true); // Locked
            ILI9341_Draw_Phone(10, 10, 20, true); // Phone present

            // Draw notification icon
            // (If you have a notification icon drawing function)
            // ILI9341_Draw_Notification(160, 50, 30, RED);
        }
        break;
    ...
    case EMERGENCY_OPEN:
        ...
        if (full_refresh) {
            ...
            // Draw warning icon or indicator
            // (If you have a warning icon drawing function)
            // ILI9341_Draw_Warning(160, 60, 40, RED);
        }
        break;
